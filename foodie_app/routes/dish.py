...
@dish_bp.route("/api/v1/restaurants/<int:restaurant_id>/dishes", methods=["GET", "POST"])
def handle_dishes(restaurant_id):
    ...
    for restaurant in restaurants:
        if restaurant["id"] == restaurant_id:
            ...
            if request.method == "GET":
                return jsonify(restaurant["dishes"]), 200
        ...
        if restaurant["id"] == restaurant_id:
            for dish in restaurant["dishes"]:
                if dish["id"] == dish_id:
                    ...
                    return jsonify({"message": "Dish disabled"}), 200
    return jsonify({"error": "Dish not found"}), 404
